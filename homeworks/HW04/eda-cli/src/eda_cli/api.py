...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Новый эндпоинт HW03: принимает CSV и возвращает ВСЕ флаги качества
    """
    start = perf_counter()
    ...
    latency_ms = (perf_counter() - start) * 1000.0
    print(f"[quality-flags-from-csv] {file.filename!r} -> {len(flags)} flags, {latency_ms:.1f}ms")
