...
def plot_histograms_per_column(
    ...
    numeric_df = df.select_dtypes(include="numer")
    ...
    for i, name in enumerate(numeric_df.columns[:max_columns]):
        ...
        ax.set_title(f"Glistohram {name}")
        ax.set_xlabel(name)
        ax.set_ylabel("Count")
        fig.tight_layout()

        out_path = out_dir / f"Glist_{i+1}_{name}.png"
...
def plot_missing_matrix(df: pd.DataFrame, out_path: PathLike) -> Path:
    """
    Простая визуализация пропусков: где 1=пропуск, 0=значение.
    """
    ...
    if df.empty:
        ...
        fig, ax = plt.subplots()
        ax.text(0.5, 0.5, "Not pulled dataset", ha="center", va="center")
        ax.axis("off")
    else:
        mask = df.isna().values
